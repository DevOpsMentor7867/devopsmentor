...
const updateNginxConfig = async (userSocketId, jenkinsPort) => {
    ...
    const locationBlock = `
          location /${userSocketId}/jenkins/ {
              proxy_pass http://127.0.0.1:${jenkinsPort}/${userSocketId}/jenkins/;
    ...
    // Find the position of the closing '}' of the server block
    const serverBlockStart = defaultConfigContent.indexOf("server {");
    const serverBlockEnd = defaultConfigContent.indexOf("}", serverBlockStart);
  
    if (serverBlockEnd !== -1) {
      // Insert location block before the end of the server block
      defaultConfigContent =
        defaultConfigContent.slice(0, serverBlockEnd) +
        locationBlock +
        defaultConfigContent.slice(serverBlockEnd);
      ...
      console.error(
        "Could not find the end of the server block in the default config"
      );
    }
  };
...
const removeNginxConfig = async (userSocketId) => {
  ...
  const locationRegex = new RegExp(
    `\\s*location /${userSocketId}/jenkins/ \{[^}]*\}`,
    "g"
  );
  ...
  jenkinsNamespace.on("connection", async (socket) => {
    ...
    socket.on("disconnect", async () => {
      ...
      if (storedContainerId) {
        const containerToStop = dockerClient.getContainer(
          JSON.parse(storedContainerId).containerId
        );
        ...
        console.log(
          `Container ${
            JSON.parse(storedContainerId).containerId
          } stopped and removed for socket ID: ${socket.id}`
        );
      }
